...
void SetCurrentTestName(const char *name) { CurrentNameRef() = name; }

const char *GetCurrentTestName() { return CurrentNameRef(); }
...
[[noreturn]] void Skip(const std::string &why) { throw SkipException{why}; }

std::string FormatEqFailure(const char *file, int line, const char *aExpr,
                            const char *bExpr, const std::string &a,
                            const std::string &b) {
  char buf[1024];
  std::snprintf(buf, sizeof(buf),
                "%s:%d: EXPECT_EQ failed: %s != %s (lhs=%s rhs=%s)",
                file ? file : "<file>", line, aExpr ? aExpr : "<a>",
                bExpr ? bExpr : "<b>", a.c_str(), b.c_str());
