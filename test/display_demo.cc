...
void EmitPrint(std::vector<uint32_t> *prog, uint32_t uartBase,
               const std::string &msg) {
  // r1 = uartBase
  toy::Emit(prog,
            toy::Lui(1, static_cast<uint16_t>((uartBase >> 16) & 0xffffu)));
...
int main(int argc, char **argv) {
  ...
  toy::Emit(&prog, toy::Lui(11, static_cast<uint16_t>((keyStatusAddr >> 16) &
                                                      0xffffu)));
  toy::Emit(&prog,
            toy::Ori(11, static_cast<uint16_t>(keyStatusAddr & 0xffffu)));

  // r4 = uartBase
  toy::Emit(&prog,
            toy::Lui(4, static_cast<uint16_t>((uartBase >> 16) & 0xffffu)));
