...
MemAccess MakeAccess(uint64_t address, uint32_t size, MemAccessType type,
                     uint64_t data = 0) {
  MemAccess access;
...
TEST(device_memory_rw) {
  ...
  ASSERT_EQ(wr.error, MemErrorType::None);

  MemAccess r = MakeAccess(4, 4, MemAccessType::Read);
  MemResponse rr = ram.read(r);
  ASSERT_EQ(rr.error, MemErrorType::None);
...
TEST(device_memory_oob) {
  ...
  MemResponse rr = ram.read(r);
  EXPECT_EQ(rr.error, MemErrorType::AccessFault);
}
...
TEST(device_memory_rom_write_fault) {
  ...
  EXPECT_EQ(wr.error, MemErrorType::AccessFault);
}
...
TEST(device_uart_status_rx) {
  ...
  ASSERT_EQ(r0.error, MemErrorType::None);
  EXPECT_TRUE((r0.data & 0x1u) == 0);

  uart.pushRx('A');
  MemResponse r1 = uart.read(status);
  ASSERT_EQ(r1.error, MemErrorType::None);
...
TEST(device_uart_invalid_access) {
  ...
  EXPECT_EQ(r.error, MemErrorType::AccessFault);
}
...
TEST(device_display_headless_regs) {
  ...
  ASSERT_EQ(w.error, MemErrorType::None);
  ...
  ASSERT_EQ(h.error, MemErrorType::None);
...
TEST(device_display_keyboard_queue) {
  ...
  ASSERT_EQ(k0.error, MemErrorType::None);
  ASSERT_EQ(k1.error, MemErrorType::None);
  EXPECT_EQ(static_cast<uint32_t>(k0.data & 0xffffffffu),
            static_cast<uint32_t>('A'));
  EXPECT_EQ(static_cast<uint32_t>(k1.data & 0xffffffffu),
            static_cast<uint32_t>('B'));

  MemAccess keyLast = MakeAccess(0x28, 4, MemAccessType::Read);
  MemResponse kl = display.read(keyLast);
  ASSERT_EQ(kl.error, MemErrorType::None);
  EXPECT_EQ(static_cast<uint32_t>(kl.data & 0xffffffffu),
            static_cast<uint32_t>('B'));

  MemAccess clear = MakeAccess(0x24, 4, MemAccessType::Write, 0u);
  MemResponse cl = display.write(clear);
  ASSERT_EQ(cl.error, MemErrorType::None);
...
TEST(device_sync_threshold) {
  ...
  public:
    ...
    MockDevice() {
      setTickHandler([this](uint64_t cycles) { mTickedCycles += cycles; });
