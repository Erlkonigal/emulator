...
TEST(integration_uart_ok) {
  ...
  ASSERT_EQ(rc, 0);
  bool check = testutil::CheckLastError(CpuErrorType::None);
  EXPECT_TRUE(check);
...
TEST(integration_ram_rw) {
  ...
  EXPECT_TRUE(testutil::CheckLastError(CpuErrorType::None));
...
TEST(integration_unmapped_fault) {
  ...
  int rc = testutil::RunEmuWithRom(romPath, false, &err);
  ASSERT_EQ(rc, 1);
  EXPECT_TRUE(testutil::CheckLastError(CpuErrorType::Stop));
}
