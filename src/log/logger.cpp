...
void formatMessage(logging::Level level, const char *file, int line,
  ...
  int pos = std::snprintf(buffer, sizeof(buffer), "[%s] [%s] %s:%d: ", timeBuf,
                          ::levelToString(level), extractFilename(file), line);
...
void init(const Config &config) { gBackend.initialize(config); }
...
const char *levelToString(Level level) { return ::levelToString(level); }
