...
void DebugServer::broadcastLog(const char *message) {
...
bool DebugServer::start(uint16_t port) {
  ...
  if (mListenFd < 0) {
    ERROR("DebugServer: failed to create socket: %s", strerror(errno));
    return false;
  }
  ...
  if (bind(mListenFd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
    ERROR("DebugServer: failed to bind port %u: %s", port, strerror(errno));
    close(mListenFd);
    mListenFd = -1;
    return false;
  }
  ...
  if (listen(mListenFd, 1) < 0) {
    ERROR("DebugServer: failed to listen: %s", strerror(errno));
    close(mListenFd);
    mListenFd = -1;
    return false;
  }
  ...
  mRunning.store(true);
  mThread = std::thread(&DebugServer::serverThread, this);

  Logger::getInstance().setHandler(
      [this](const char *msg) { broadcastLog(msg); });
  CommitThread::getInstance().iTrace.setHandler(
      [this](const char *msg) { broadcastLog(msg); });

  INFO("DebugServer: listening on port %u", port);
  return true;
}
...
void DebugServer::stop() {
  ...
  CommitThread::getInstance().iTrace.setHandler(nullptr);
  if (mThread.joinable()) {
    mThread.join();
  }
  {
...
void DebugServer::serverThread() {
  while (mRunning.load()) {
    ...
    int clientFd =
        accept(mListenFd, (struct sockaddr *)&clientAddr, &clientLen);
    ...
    if (clientFd < 0) {
      ...
      if (mRunning.load()) {
        WARN("DebugServer: accept failed: %s", strerror(errno));
      }
      continue;
    }
    ...
    INFO("DebugServer: client connected from %s:%u", clientIp,
    ...
    INFO("DebugServer: client disconnected");
  }
}

void DebugServer::handleClient(int clientFd) {
  ...
  sendLine(clientFd, "DebugServer connected. Type 'help' for commands.");
