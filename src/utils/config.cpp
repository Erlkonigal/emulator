...
namespace {
bool applyConfigValue(EmulatorConfig *config, const std::string &key,
  ...
  if (key == "mtrace") {
    ...
    if (!parseBool(value, &flag)) {
      if (error != nullptr)
        *error = "Invalid mtrace value: " + value;
      return false;
    }
    config->mTrace = flag;
    return true;
  ...
  if (key == "bptrace") {
    ...
    if (!parseBool(value, &flag)) {
      if (error != nullptr)
        *error = "Invalid bptrace value: " + value;
      return false;
    }
    config->bpTrace = flag;
    return true;
...
bool EmulatorConfig::loadConfigFile(const std::string &path, bool required,
