...
void EmulatorConfig::printUsage(const char *exe) {
  ...
  std::fprintf(
      ...
      "  --log-level <lvl>     Set log level (trace, debug, info, warn, "
      "error)\n"
      "  --log-filename <path> Set log file path (device->name.out, "
      "other->name.err)\n"
...
bool EmulatorConfig::findConfigPath(int argc, char **argv,
                                    EmulatorConfig *config, bool *required,
                                    std::string *error) {
...
bool EmulatorConfig::parseArgs(int argc, char **argv, EmulatorConfig *config,
                               std::string *error) {
  if (config == nullptr) {
    return false;
  }
