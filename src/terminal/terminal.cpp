...
void Terminal::startRefreshThread() {
  mRefreshRunning = true;
  mRefreshThread = std::thread([this]() {
    while (mRefreshRunning) {
      renderAll();
...
void Terminal::stopRefreshThread() {
  mRefreshRunning = false;
  if (mRefreshThread.joinable()) {
    mRefreshThread.join();
  }
}
...
void Terminal::runCursesInputLoop() {
  ...
  while (!mShouldClose) {
    ...
    int ch =
        (mFocus == FocusPanel::VTERM) ? wgetch(currentWin) : wgetch(mDebugWin);
